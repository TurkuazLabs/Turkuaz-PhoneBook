...
# 📌 Amac: Turkuaz PhoneBook v2.39.0 release oncesi ve sonrasi dogrulama adimlarini tek yerde tutar.
# 📌 Modul - Markdown
# Version: 1.4.0
...
# Turkuaz PhoneBook v2.39.0 Release Checklist
...
- Teknik surumler `app 2.39.0`, `launcher 2.5.2`, Android/iOS build 590 ile version consistency gate'i gecmeli.
- `updates/update-manifest.yml` main branchte son yayinlanmis release referansi olarak kalabilir; gercek v2.39.0 cross-platform manifesti tag-triggered Release workflow tarafindan release assetlerinin gercek SHA-256 degerleriyle uretilir.

## Yayin

1. Son yesil `main` commitinde `v2.39.0` tag olustur.
2. Tag'i GitHub'a push et.
3. `Release` workflow'un tum joblarinin yesil tamamlanmasini bekle.
4. GitHub Release basliginin `Turkuaz PhoneBook v2.39.0` oldugunu dogrula.
...
- `update-manifest.yml`
- `CHECKSUMS.txt`
- `TelefonRehberi-Portable-v2.39.0-FULL.zip`
- `TelefonRehberi-Setup-v2.39.0.exe`
- `TelefonRehberi-Linux-v2.39.0-FULL.tar.gz`
- `Turkuaz-PhoneBook-Android-v2.39.0-debug.apk`

## Release sonrasi

- `CHECKSUMS.txt` ile release assetlerinin SHA-256 degerlerini dogrula.
- Release `update-manifest.yml` icinde `app_version: "2.39.0"` ve `launcher_version: "2.5.2"` oldugunu dogrula.
...
- Turkce ve English masaustu dil tercihlerinin kaydedildigi anda ayni pencere icinde restart olmadan uygulandigini kontrol et.
- Launcher latest release kontrolunun v2.39.0 manifestini gordugunu dogrula.
