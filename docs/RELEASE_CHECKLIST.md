...
# 📌 Modul - Markdown
# Version: 1.6.0
# Aciklama: config/version.yml merkezli preflight, dinamik tag/release, checksum, otomatik main update-manifest finalizasyonu ve dagitim smoke kontrolleridir.
...
- Release tamamlandiktan sonra `sync-published-manifest.sh` tarafindan olusturulan `chore(release): finalize v<APP_VERSION> published metadata` commitini ve main `updates/update-manifest.yml` dosyasinin release assetleriyle ayni SHA-256 degerlerini tasidigini dogrula.
