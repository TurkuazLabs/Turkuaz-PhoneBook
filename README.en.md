<div align="center">
  ...
  <p><a href="README.md">Türkçe README</a></p>
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/version-2.41.0-0aa6a6" alt="Version 2.41.0">
...
- Turkuaz Theme v3 with a rebuilt Light/Dark color system and accessible button states
- Central semantic theme tokens, 2px keyboard focus rings and WCAG contrast quality gates
...
## v2.40.1 Automatic Release Manifest Patch

v2.40.1 automates post-release metadata finalization. After a GitHub Release is published, the generated `update-manifest.yml` is synchronized to `main/updates/update-manifest.yml` through the GitHub Contents API only when the release tag still matches `main/config/version.yml`. If main has already advanced to a newer version, the older release metadata is skipped.
...
v2.39.2 speeds up Windows CI/release packaging. SQLite JDBC, SLF4J, FlatLaf and Temurin JRE downloads are reused through a GitHub Actions cache keyed by the `launcher.yml` dependency configuration. Every cached binary is verified again with SHA-256 before use; a mismatch is deleted and downloaded again from its trusted source.

## v2.39.1 Reminder Notification Patch

v2.39.1 prevents the same desktop reminder summary from being shown repeatedly when the app is reopened on the same day. Only a successfully displayed notification date is persisted in the user config area; if the SystemTray notification fails, a later launch can retry it.
