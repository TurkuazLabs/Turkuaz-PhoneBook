...
The canonical artwork remains language-neutral. At runtime, the Swing sidebar/About brand mark is recolored from the same alpha artwork to match the active Light or Dark palette. Windows packaging also generates `app-icon-light.ico` and `app-icon-dark.ico`; the native splash selects the matching variant before Java starts.
